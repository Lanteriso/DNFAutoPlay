...
def GetPlayerRoom(player):
    FindImg = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/MiniMapBoss.PNG', 0.7, 1, None, [[25, 200], ]], ])
    FindImg1 = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/MiniMapPlayer.PNG', 0.7, 1, None, [[25, 200], 'right']], ])
    if FindImg and FindImg1:
        return MiniMap.获取玩家当前房间坐标(FindImg[5][0], FindImg1[5][0])

def GetTarget(player):
    print('【0】 GetTarget')
    FindListNone = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/MiniMapNone.PNG', 0.7, 1, None, [[0, 0], 'right']], ])
    FindList['MiniMapBoss'] = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/MiniMapBoss.PNG', 0.7, 1, None, [[25, 200], ]], ])
    FindImg5 = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/zctz.PNG', 0.7, 1, None, [[36, 13], ]], ])
    FindList['MiniMapPlayer'] = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/MiniMapPlayer.PNG', 0.7, 1, None, [[0, 0], ]],['resources/destroyedcastleofdead/MiniMapPlayer1.PNG', 0.7, 1, None, [[0, 0], ]],['resources/destroyedcastleofdead/MiniMapPlayer2.PNG', 0.7, 1, None, [[18, 0], ]], ])
    if FindList['MiniMapBoss'] and FindList['MiniMapPlayer']:
        player.currentroomlist2 = MiniMap.获取玩家当前房间坐标3(FindList['MiniMapBoss'], FindList['MiniMapPlayer'])
        player.currentroomlist = MiniMap.获取玩家当前房间坐标2(FindList['MiniMapBoss'], FindList['MiniMapPlayer'])
        player.状态 = "副本中"
    elif FindImg5:
        player.状态 = "结算中"

def GetTargetInDungeon(player):
    FindList['MiniMapNone'] = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/MiniMapNone.PNG', 0.7, 1, None, [[0, 0], 'right']], ])
    FindList['MiniMapPlayer'] = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/MiniMapPlayer.PNG', 0.7, 1, None, [[0, 0],]],['resources/destroyedcastleofdead/MiniMapPlayer1.PNG', 0.7, 1, None, [[0, 0],]], ['resources/destroyedcastleofdead/MiniMapPlayer2.PNG', 0.7, 1, None, [[18, 0],]], ])
    FindImg5 = myfunction.试验查找指定图片([0, 0, 1920, 1080],[['resources/destroyedcastleofdead/zctz.PNG', 0.7, 1, None, [[36, 13], ]], ])
...
def 可进入下一个房间(player):
    while 1:
        ...
        FindImg1 = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/0.PNG', 0.6, 1.62, None, [[70, player.身高], 'right']],['resources/destroyedcastleofdead/0r.PNG', 0.6, 1.62, None, [[1, player.身高], 'right']],['resources/destroyedcastleofdead/0l.PNG', 0.6, 1.62, None, [[70, player.身高], 'right']], ])
...
def 战斗进行中(player):
    while 1:
        ...
        FindImg5 = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/0.PNG', 0.6, 1.62, None, [[70, player.身高], 'right']],['resources/destroyedcastleofdead/0r.PNG', 0.6, 1.62, None, [[1, player.身高], 'right']],['resources/destroyedcastleofdead/0l.PNG', 0.6, 1.62, None, [[70, player.身高], 'right']], ])
...
def 因找不到敌人而移动(player):
    ...
    FindImg1 = myfunction.试验查找指定图片([0, 0, 1920, 1080], [['resources/destroyedcastleofdead/0.PNG', 0.6, 1.62, None, [[70, player.身高], 'right']],['resources/destroyedcastleofdead/0r.PNG', 0.6, 1.62, None, [[1, player.身高], 'right']],['resources/destroyedcastleofdead/0l.PNG', 0.6, 1.62, None, [[70, player.身高], 'right']], ])
    FindImg2 = myfunction.试验查找指定图片([0, 0, 1920, 1080], player.currentroomlist2)
    if FindImg1 and FindImg2:
        AIMoveTo(FindImg1,FindImg2,(0,0))
